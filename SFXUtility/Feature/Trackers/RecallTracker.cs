	...
	internal class RecallTracker : Base
	{
		...
		private void OnDraw(EventArgs args)
		{
			try
			{
				...
				{
					if (recall.LastStatus != Class.RecallPacket.RecallStatus.Unknown)
					{
		...
		private void OnGameProcessPacket(GamePacketEventArgs args)
		{
			try
			{
				...
				if (args.PacketData[0] == Class.RecallPacket.Header)
				{
					...
					if (packet.Status == Class.RecallPacket.RecallStatus.RecallFinished)
					{
						IoC.Resolve<Mediator>().NotifyColleagues(Name + "_Finished", packet.UnitNetworkId);
					}
					if (packet.Status == Class.RecallPacket.RecallStatus.RecallStarted)
					{
						IoC.Resolve<Mediator>().NotifyColleagues(Name + "_Started", packet.UnitNetworkId);
					}
					if (packet.Status == Class.RecallPacket.RecallStatus.RecallAborted)
					{
						IoC.Resolve<Mediator>().NotifyColleagues(Name + "_Aborted", packet.UnitNetworkId);
					}
					if (packet.Status == Class.RecallPacket.RecallStatus.Unknown)
					{
		...
		private class Recall
		{
			...
			private Class.RecallPacket.RecallStatus _lastStatus;
			...
			{
				Hero = hero;
				LastStatus = Class.RecallPacket.RecallStatus.Unknown;
			}
			...
			public Class.RecallPacket.RecallStatus LastStatus
			{
				get { return _lastStatus; }
				set
				{
					_lastStatus = value;
					_recallStart = _lastStatus == Class.RecallPacket.RecallStatus.RecallStarted ||
						_lastStatus == Class.RecallPacket.RecallStatus.TeleportStart
						? Game.Time
						: 0f;
			...
			{
				if (LastStatus == Class.RecallPacket.RecallStatus.RecallStarted ||
				    LastStatus == Class.RecallPacket.RecallStatus.TeleportStart)
					return Color.Beige;
				if (LastStatus == Class.RecallPacket.RecallStatus.RecallFinished ||
				    LastStatus == Class.RecallPacket.RecallStatus.TeleportEnd)
					return Color.GreenYellow;
				if (LastStatus == Class.RecallPacket.RecallStatus.RecallAborted ||
				    LastStatus == Class.RecallPacket.RecallStatus.TeleportAbort)
					return Color.Red;
				return Color.Black;
			}
			...
			public bool Update()
			{
				var additional = LastStatus == Class.RecallPacket.RecallStatus.RecallStarted ||
					LastStatus == Class.RecallPacket.RecallStatus.TeleportStart
					? Duration + 20f
					: 20f;
